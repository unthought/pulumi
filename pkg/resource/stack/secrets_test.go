...
func TestCachingCrypter(t *testing.T) {
	...
	foo1Ser, err := SerializePropertyValue(foo1, enc /* showSecrets */, false)
	assert.NoError(t, err)
	...
	foo2Ser, err := SerializePropertyValue(foo2, enc /* showSecrets */, false)
	assert.NoError(t, err)
	...
	barSer, err := SerializePropertyValue(bar, enc /* showSecrets */, false)
	assert.NoError(t, err)
	...
	// encrypted.
	foo1Ser2, err := SerializePropertyValue(foo1, enc /* showSecrets */, false)
	assert.NoError(t, err)
	assert.Equal(t, 3, sm.encryptCalls)
	...
	// encrypted.
	foo2Ser2, err := SerializePropertyValue(foo2, enc /* showSecrets */, false)
	assert.NoError(t, err)
	assert.Equal(t, 3, sm.encryptCalls)
	assert.Equal(t, foo2Ser, foo2Ser2)

	// Serialize "bar" again. Encrypt should not be called, as this value has already been encrypted.
	barSer2, err := SerializePropertyValue(bar, enc /* showSecrets */, false)
	assert.NoError(t, err)
	assert.Equal(t, 3, sm.encryptCalls)
	...
	foo1Ser2, err = SerializePropertyValue(foo1Dec, enc /* showSecrets */, false)
	assert.NoError(t, err)
	assert.Equal(t, 3, sm.encryptCalls)
	assert.Equal(t, foo1Ser, foo1Ser2)
	...
	// Serialize the second copy of "foo" again. Encrypt should not be called, as this value has already been
	// cached by the earlier calls to Decrypt.
	foo2Ser2, err = SerializePropertyValue(foo2Dec, enc /* showSecrets */, false)
	assert.NoError(t, err)
	assert.Equal(t, 3, sm.encryptCalls)
	assert.Equal(t, foo2Ser, foo2Ser2)
	...
	barSer2, err = SerializePropertyValue(barDec, enc /* showSecrets */, false)
	assert.NoError(t, err)
	assert.Equal(t, 3, sm.encryptCalls)
	assert.Equal(t, barSer, barSer2)
}
