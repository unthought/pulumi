...
func TestDeploymentSerialization(t *testing.T) {
	...
	dep, err := SerializeResource(res, config.NopEncrypter /* showSecrets */, false)
...
func TestCustomSerialization(t *testing.T) {
	...
	t.Run("SerializeProperties", func(t *testing.T) {
		serializedPropMap, err := SerializeProperties(propMap, config.BlindingCrypter /* showSecrets */, false)
		assert.NoError(t, err)
