		...
		case engine.ResourcePreEvent:
			...
			if m := e.Payload.(engine.ResourcePreEventPayload).Metadata; shouldShow(m, opts) || isRootStack(m) {
				...
				if m.Old != nil {
					...
					res, err := stack.SerializeResource(oldState, config.NewPanicCrypter() /* showSecrets */, false)
					if err == nil {
						step.OldState = &res
					} else {
				...
				if m.New != nil {
					...
					res, err := stack.SerializeResource(newState, config.NewPanicCrypter() /* showSecrets */, false)
					if err == nil {
						step.NewState = &res
					} else {
